  ...
  <section class="page">
    ...
    <div v-else class="cart-panel">
      <div class="cart-item" v-for="item in cartItems" :key="item.id">
        ...
        <button class="action danger" @click="remove(item.id)">移除</button>
      </div>
...
const submitOrder = () => {
  const order = userStore.checkout()
  if (order) {
    successMessage.value = `订单提交成功（编号：${order.id}），共 ${order.items.length} 件商品。`
    setTimeout(() => {
