<template>
  <section class="page">
    ...
    <div v-else class="favorite-list">
      <article class="favorite-card" v-for="item in favorites" :key="item.id">
        <img :src="resolveImage(item.imageUrl)" alt="" class="thumb">
        <div class="info">
          <h3>{{ item.name }}</h3>
          <p class="price">￥{{ item.price }}</p>
          <div class="actions">
            <router-link class="link" :to="`/book/${item.id}`">查看详情</router-link>
            <button class="btn" @click="addCart(item)">加入购物车</button>
            <button class="btn ghost" @click="remove(item.id)">取消收藏</button>
          </div>
        </div>
      </article>
    </div>
...
const addCart = (item) => {
  userStore.addToCart({ ...item, quantity: 1 })
}
