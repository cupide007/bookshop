...
<template>
    <section class="page">
      ...
      <div v-else class="book-list-container">
        <div class="book-item" v-for="book in books" :key="book.productId || book.id">
          <router-link :to="`/book/${book.productId || book.id}`">
            <img :src="resolveImage(book.imageUrl)" alt="" />
            <p class="name">{{ book.name || book.title }}</p>
          </router-link>
...
const fetchData = () => {
  ...
  axios.get(requestUrl)
    .then(res => {
      books.value = res.data?.data?.list || res.data?.data || res.data || []
    })
    .catch(err => {
